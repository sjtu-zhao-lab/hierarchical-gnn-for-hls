...
def create_graph(kernel, arr_cfgs, loop_cfgs, latencies, DSPs, LUTs, FFs, ILs, IIs):
    ...
    loopcfg2res, loopcfg2latency = get_subloop_config(kernel)
    ...
    for idx in tqdm(range(len(loop_cfgs))):
        ...
        add_supernode_to_c(code_filepath, loop_cfg, kernel)
        graph = generate_IR_graph()
        
        # Remove the function nodes
        remove_function_nodes(graph)
        
        # Match super node resource and latency
        supernode1_feature = get_supernode_feature(kernel, arr_cfg, loop_cfg, 0, loopcfg2res, loopcfg2latency)
        supernode2_feature = get_supernode_feature(kernel, arr_cfg, loop_cfg, 1, loopcfg2res, loopcfg2latency)
        
        # If cannot match, drop the case
        if supernode1_feature is None or supernode2_feature is None:
            continue
        
        # Annotate the graph with various features
        add_memory_flow(graph, arr_cfg)
        add_resource_feature(graph, supernode1_feature['resource'], supernode2_feature['resource'])
        add_latency_feature(graph, supernode1_feature['latency'], supernode2_feature['latency'], supernode1_feature['tc'], supernode2_feature['tc'])
        add_degree_feature(graph)
        
        # Change the call node to super node
        change_call_node_type(graph)
...
def build_dataset(graphs):
    ...
    data_list = []
    node_attrs = ['text', 'type', 'latency', 'dsp', 'lut', 'ff', 'in_degree', 'out_degree']
